...
const registerFunc = () => {
    ...
    (data) => {
        if (data.registerFlag === false && data.dbFlag === false) {
            ...
            // window.location = "login.jsp?username=" + data.user.username + "&password=" + data.user.password;
            window.location = "user/registerToLoginServlet?username=" + data.user.username + "&password=" + data.user.password;
        }
